...
def calc_iou(a, b):
    # Expand dims for broadcasting instead of unsqueeze
    a = a[:, None, :]  # (N, 1, 4)
    b = b[None, :, :]  # (1, M, 4)
...
#                 focal_weight = (1. - alpha_factor) * torch.pow(classification, gamma)
...
#             # Focal Loss 계산
#             alpha_factor = torch.where(torch.eq(targets, 1.), alpha, 1. - alpha)
